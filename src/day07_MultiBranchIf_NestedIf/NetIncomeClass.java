package day07_MultiBranchIf_NestedIf;

public class NetIncomeClass {
    public static void main(String[] args) {

        int salary = 100000;
        boolean isMarried = true;
        double taxRate = 0; //Assign a temporary value until you have your final value

        if (salary >= 135000) {
            taxRate = 0.35;
        }
        if (salary >= 100000 && salary < 130000) { //this says if the salary is between 100-129k the tax rate is 30%
            taxRate = 0.3;
        }
        if (salary >= 80000 && salary < 100000) {
            taxRate = 0.25;
        }
        if (salary < 80000) {
            taxRate = .20;
        }
        if (isMarried) { //if the person is married, tax is reduced by 5%
            taxRate -= .05;
        }

        double netIncome = salary *(1-taxRate);

        System.out.println("netincome = " + netIncome);

        /*
            3. Create a class named NetIncomeCalc, Write a program that can calculate the salary after tax based on the following requirements
                the tax rates are:
                        35% for salary of 130K or more
                        30% for salary of 100K to 129k
                        25% for salary of 80K to 99K
                        20% for salary of 79K or less

                in addition:
                	if the person is married, he/she will pay 5% less tax
         */


    }
}
